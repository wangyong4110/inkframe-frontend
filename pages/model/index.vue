...
function onProviderSelect() {
  ...
  // Azure deployment names are account-specific; skip auto-fill for api_version
  if (opt.apiVersionHint && !providerForm.value.api_version && opt.name !== 'azure') {
...
const editModelModal = ref<{
  ...
  rateLimit: number
  apiVersion: string
}>({ show: false, saving: false, groupKey: 0, modelId: 0, name: '', availableModels: [], displayName: '', type: '', maxTokens: 0, quality: 0.8, timeout: 0, concurrency: 0, rateLimit: 0, apiVersion: '' })
...
function openEditModel(groupKey: number, m: AIModel) {
  ...
  editModelModal.value = {
    ...
    apiVersion: (m as any).api_version ?? '',
  }
}
...
async function handleEditModel() {
  ...
  try {
    const updated = await updateModel(e.modelId, {
      ...
      api_version: e.apiVersion || undefined,
    })
...
const addModelModal = ref<{
  ...
  availableModels: string[]
}>({ show: false, providerId: 0, groupKey: 0, providerName: '', name: '', displayName: '', type: '', quality: 0.8, maxTokens: 0, timeout: 0, concurrency: 0, rateLimit: 0, apiVersion: '', saving: false, loadingModels: false, availableModels: [] })
...
async function handleCreateModelModal() {
  ...
  try {
    ...
    await createModel({
      ...
      api_version: m.apiVersion || undefined,
    })
    toast.success('模型已添加')
    closeAddModelModal()
            ...
            <div class="px-6 py-5 space-y-4 overflow-y-auto flex-1">
              ...
              <div v-if="providerForm.name === 'azure'" class="rounded-lg bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-700 px-3 py-2.5 flex items-start gap-2">
                ...
                <div class="text-xs text-sky-700 dark:text-sky-300 space-y-1">
                  ...
                  <div><span class="font-medium">API Version：</span>在对应<span class="font-medium">模型</span>上单独设置（非提供商级别）</div>
                </div>
              </div>
              <!-- Ollama 无 Key 提示 -->
